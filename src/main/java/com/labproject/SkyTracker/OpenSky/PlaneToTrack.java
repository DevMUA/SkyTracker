...
public class PlaneToTrack {

    @Id
    @SequenceGenerator(
            name = "planetotrack_sequence",
            sequenceName = "planetottrack_sequence",
            allocationSize = 1
    )
    ...
    public PlaneToTrack(String icao24, String callsign, String origin_country, int time_position,
    ...
    public PlaneToTrack(Long id, String icao24, String callsign, String origin_country, int time_position,
                        int last_contact, float longitude, float latitude, float baro_altitude,
                        boolean on_ground, float velocity, float true_track, float vertical_rate,
                        int[] sensors, float geo_altitude, String squawk, boolean spi, int position_source) {
    ...
    public void convertPlane(Plane p){
        this.icao24 = p.getIcao24();
        this.callsign = p.getCallsign();
        this.origin_country = p.getOrigin_country();
        this.time_position = p.getTime_position();
        this.last_contact = p.getLast_contact();
        this.longitude = p.getLongitude();
        this.latitude = p.getLatitude();
        this.baro_altitude = p.getBaro_altitude();
        this.on_ground = p.isOn_ground();
        this.velocity = p.getVelocity();
        this.true_track = p.getTrue_track();
        this.vertical_rate = p.getVertical_rate();
        this.sensors = p.getSensors();
        this.geo_altitude = p.getGeo_altitude();
        this.squawk = p.getSquawk();
        this.spi = p.isSpi();
        this.position_source = p.getPosition_source();
    }
